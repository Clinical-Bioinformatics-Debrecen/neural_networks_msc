...
# --- MODEL ---
class SpectrogramDenseNet(nn.Module):
    def __init__(self, num_classes):
        super().__init__()
        self.backbone = models.densenet121(weights=models.DenseNet121_Weights.IMAGENET1K_V1)
        # Keep the original conv0 for 3-channel input
        # Replace classifier
...
model = SpectrogramDenseNet(num_classes).to(device)
...
for epoch in range(num_epochs):
    ...
    if val_acc > best_val_acc:
        ...
        epochs_no_improve = 0
        torch.save(model.state_dict(), 'densenet121_bird_best.pth')
    else:
        epochs_no_improve += 1
...
torch.save(model.state_dict(), 'densenet121_bird_last.pth')
